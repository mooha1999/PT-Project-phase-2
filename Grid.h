...
// ========= Setters and Getters Functions =========
	...
	//=================================================================================================
	// NEW ADDED functions
	void  SetCurretnPlayer(int x);


	Player* GetNextPlayer(const CellPosition& position);  //  Gets a Pointer to the first Player after the passed "position"
	Player* getPoorestPlayer();								//  Gets a pointer to the player with the least amount of money	
	Cell* GetCellFromPosition(CellPosition cPos);           // Give a cellposition return a pointer to the game cell


	void SaveAll(ofstream& OutFile, GObjectsTypes Type);
	void LoadAll(ifstream& inFile, GObjectsTypes Type, int ObjectsCount);
	Ladder* CurrentCellLadder(const CellPosition& position); //  Checks if the current cell (position) has a ladder and returns it	
	snake* CurrentCellSnake(const CellPosition& position);   //  Checks if the current cell (position) has a Snake and returns it	
	snake* GetNextSnake(const CellPosition& position);
	Card* CurrentCellCard(const CellPosition& position);     //  Checks if the current cell (position) has a Card and returns it
